import { useCallback, useEffect, useState } from "react";

import { sendToWorker } from "../lib/messages";
import type { HostStatus, MemoryRecord } from "../lib/messages";
import {
  getSettings,
  setSettings,
  DEFAULT_SETTINGS,
  type Settings,
  type SiteId,
} from "../lib/settings";
import { SITE_CONFIGS } from "../lib/site-config";

const INSTALL_CMD = "npx -p @sovseal/mcp-server sovseal-install-native-host";

const Logo = () => (
  <svg viewBox="3 3 351 245" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path
      d="M151.615 16.548 C 174.979 20.622,200.000 33.253,200.000 40.974 C 200.000 47.495,193.144 50.261,187.664 45.950 C 183.266 42.491,166.641 35.086,158.354 32.896 C 148.408 30.267,129.820 29.306,120.411 30.935 C 98.704 34.693,82.650 45.471,75.054 61.387 C 71.160 69.546,70.051 74.763,70.036 85.000 C 70.017 97.942,73.453 108.312,81.446 119.433 C 83.844 122.770,97.306 136.949,111.362 150.943 C 125.417 164.936,137.406 177.566,138.004 179.009 C 140.836 185.847,134.863 189.899,114.286 195.096 C 92.159 200.684,69.031 200.092,49.084 193.425 C 43.355 191.511,38.260 190.080,37.763 190.246 C 36.595 190.635,41.376 196.595,46.500 201.137 C 61.624 214.544,84.097 220.667,114.500 219.665 C 138.876 218.861,154.606 213.972,164.874 204.006 C 174.674 194.496,176.412 179.648,169.207 167.000 C 167.759 164.458,151.321 147.055,131.425 127.000 C 98.050 93.359,96.122 91.216,94.603 86.068 C 90.936 73.641,93.312 63.543,101.936 54.910 C 108.997 47.841,115.154 45.528,125.026 46.234 C 135.805 47.005,138.639 48.969,160.064 70.513 C 170.723 81.231,179.856 90.000,180.361 90.000 C 180.865 90.000,184.703 86.732,188.889 82.737 C 227.524 45.869,264.117 32.930,297.206 44.437 C 323.885 53.715,342.244 77.824,343.759 105.571 C 345.786 142.711,318.233 174.936,281.383 178.523 C 267.494 179.875,250.711 175.561,238.611 167.530 C 235.259 165.305,211.974 142.842,183.166 114.043 C 155.850 86.735,132.375 63.700,131.000 62.854 C 122.420 57.576,110.591 62.378,107.942 72.215 C 105.631 80.798,105.917 81.172,144.914 120.496 C 176.876 152.726,180.902 157.146,183.328 162.666 C 194.446 187.972,185.019 212.956,159.802 225.011 C 128.684 239.887,77.404 237.565,47.324 219.917 C 38.271 214.606,25.074 200.874,21.064 192.592 C 11.573 172.991,11.662 150.486,21.309 131.082 C 25.235 123.185,28.818 120.198,32.717 121.569 C 34.214 122.096,36.115 123.732,36.942 125.206 C 38.327 127.674,38.260 128.302,36.079 133.192 C 28.630 149.903,27.825 155.862,31.939 163.842 C 36.301 172.304,48.462 179.412,63.944 182.549 C 79.564 185.714,115.218 182.875,116.720 178.347 C 116.930 177.713,107.006 167.138,94.667 154.847 C 71.016 131.288,65.712 124.565,60.573 111.635 C 50.309 85.810,54.819 58.706,72.667 38.946 C 89.151 20.696,121.985 11.381,151.615 16.548 M261.507 56.135 C 238.946 61.415,222.105 71.263,201.250 91.374 C 196.162 96.280,192.000 100.823,192.000 101.470 C 192.000 102.116,203.813 114.356,218.250 128.669 C 242.244 152.457,245.145 155.003,252.000 158.291 C 260.927 162.572,266.380 163.943,274.614 163.976 C 289.404 164.035,302.601 158.588,313.115 148.084 C 323.835 137.376,329.000 124.993,329.000 110.000 C 329.000 84.587,312.207 62.977,287.448 56.528 C 281.037 54.859,267.819 54.658,261.507 56.135"
      fill="#0B1215"
      fillRule="evenodd"
    />
  </svg>
);

function Switch({ on, onClick }: { on: boolean; onClick: () => void }) {
  return (
    <div
      className={`switch${on ? " on" : ""}`}
      role="switch"
      aria-checked={on}
      onClick={onClick}
    >
      <span className="knob" />
    </div>
  );
}

export function App() {
  const [status, setStatus] = useState<HostStatus | null>(null);
  const [connected, setConnected] = useState<boolean | null>(null);
  const [settings, setLocalSettings] = useState<Settings>(DEFAULT_SETTINGS);
  const [recent, setRecent] = useState<MemoryRecord[]>([]);

  const refresh = useCallback(async () => {
    const [st, sx] = await Promise.all([
      sendToWorker({ kind: "STATUS" }),
      getSettings(),
    ]);
    setLocalSettings(sx);
    if (st.ok && st.kind === "STATUS") {
      setConnected(true);
      setStatus(st.status);
      const r = await sendToWorker({ kind: "RECENT", limit: 8 });
      if (r.ok && r.kind === "RECENT") setRecent(r.memories);
    } else {
      setConnected(false);
      setStatus(null);
    }
  }, []);

  useEffect(() => {
    void refresh();
  }, [refresh]);

  const toggleGlobal = async () => {
    const next = await setSettings({ autoCapture: !settings.autoCapture });
    setLocalSettings(next);
  };

  const toggleSite = async (id: SiteId) => {
    const currentlyOn = settings.sites[id] !== false;
    const next = await setSettings({
      sites: { ...settings.sites, [id]: !currentlyOn },
    });
    setLocalSettings(next);
  };

  const del = async (id: string) => {
    const r = await sendToWorker({ kind: "DELETE", memoryId: id });
    if (r.ok) {
      setRecent((prev) => prev.filter((m) => m.id !== id));
      void refresh();
    }
  };

  return (
    <div className="wrap">
      <div className="head">
        <span className="logo">
          <Logo />
        </span>
        <span className="name">sovseal</span>
        <span className="tag">local · zero-knowledge</span>
      </div>

      {/* status */}
      <div className="card status">
        <span
          className={`dot ${connected === null ? "" : connected ? "on" : "off"}`}
        />
        <div>
          <div className="label">
            {connected === null
              ? "Checking…"
              : connected
                ? "On-device engine connected"
                : "Host not connected"}
          </div>
          <div className="sub">
            {connected
              ? `project ${status?.projectId.slice(0, 8)}… · sync ${status?.syncEnabled ? "on" : "off (local-only)"}`
              : "Install the native host to enable memory."}
          </div>
        </div>
        {connected && (
          <div className="meta">
            <div style={{ fontSize: 18, color: "#fafafa", fontWeight: 600 }}>
              {status?.count ?? 0}
            </div>
            memories
          </div>
        )}
      </div>

      {/* install help */}
      {connected === false && (
        <div className="card install">
          <p>
            Run this once in your terminal, then reload this page. It registers
            the on-device memory bridge — nothing leaves your machine.
          </p>
          <code>{INSTALL_CMD}</code>
          <p style={{ marginTop: 8 }}>
            <button className="btnlink" onClick={() => void refresh()}>
              Re-check connection
            </button>
          </p>
        </div>
      )}

      {/* capture controls */}
      <div className="section-title">Capture</div>
      <div className="card">
        <div className="row">
          <div>
            <div className="rlabel">Auto-capture</div>
            <div className="rsub">Save prompts you send. Manual: ⌘/Ctrl+M.</div>
          </div>
          <div className="spacer" />
          <Switch on={settings.autoCapture} onClick={() => void toggleGlobal()} />
        </div>
        <div className="sites">
          {(Object.keys(SITE_CONFIGS) as SiteId[]).map((id) => {
            const on = settings.autoCapture && settings.sites[id] !== false;
            return (
              <div className="row" key={id}>
                <div className="rlabel">{SITE_CONFIGS[id].label}</div>
                <div className="spacer" />
                <Switch
                  on={on}
                  onClick={() => void toggleSite(id)}
                />
              </div>
            );
          })}
        </div>
      </div>

      {/* recent */}
      {connected && (
        <>
          <div className="section-title">Recent memories</div>
          <div className="card">
            {recent.length === 0 ? (
              <div className="empty">No memories yet.</div>
            ) : (
              recent.map((m) => (
                <div className="mem" key={m.id}>
                  <span className="mtext">{m.text}</span>
                  <button
                    className="mdel"
                    title="Delete"
                    onClick={() => void del(m.id)}
                  >
                    ✕
                  </button>
                </div>
              ))
            )}
          </div>
        </>
      )}

      <div className="foot">
        <span>v0.1.0</span>
        <a href="https://sovseal.com" target="_blank" rel="noreferrer">
          sovseal.com
        </a>
      </div>
    </div>
  );
}
